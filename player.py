...
class Player(Entity):
    ...
    def input(self):
        if not self.attacking and data.playing == True:
            ...
            if keys[pygame.K_e] and self.can_switch_magic == True:
    ...
    def get_status(self):
        ...
        if self.direction.x == 0 and self.direction.y == 0:
            if self.status.find('_idle') == -1 and self.status.find('_attack') == -1:
                self.status = self.status + '_idle'
        if self.attacking:
            self.direction.x = 0
            self.direction.y = 0
            if self.status.find('_attack') == -1:
                if self.status.find('_idle') != -1:
